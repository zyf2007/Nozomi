...
func (a *App) migrate(isNewDB bool) error {
	stmts := []string{
		...
		`create table if not exists upstream_providers (id integer primary key autoincrement, name text not null, type text not null, enabled integer not null default 1, weight integer not null default 100, daily_limit integer not null default 0, quota_timezone text not null default 'Asia/Shanghai', created_at text not null, updated_at text not null)`,
		...
		`create table if not exists provider_resend_config (provider_id integer primary key, api_key text not null default '', from_address text not null default '', reply_to text not null default '', foreign key(provider_id) references upstream_providers(id) on delete cascade)`,
		`create table if not exists provider_brevo_config (provider_id integer primary key, api_key text not null default '', from_address text not null default '', from_name text not null default 'Nozomi', reply_to text not null default '', foreign key(provider_id) references upstream_providers(id) on delete cascade)`,
		...
		`create table if not exists messages (id integer primary key autoincrement, downstream_account_id integer, downstream_from text, downstream_to text, subject text, raw text, text_body text, html_body text, sent_raw text not null default '', provider_id integer, provider_type text, rule_id integer, template_id integer, template_data text, status text not null, error text not null default '', provider_message_id text, callback_event text, callback_reason text, bounce_type text, created_at text not null, updated_at text not null)`,
	...
	if err := a.ensureColumn("upstream_providers", "quota_timezone", `alter table upstream_providers add column quota_timezone text not null default 'Asia/Shanghai'`); err != nil {
		return err
	}
	if err := a.ensureColumn("messages", "sent_raw", `alter table messages add column sent_raw text not null default ''`); err != nil {
		return err
	}
	...
	if isNewDB {
		...
		if _, err := a.db.Exec(`insert or ignore into upstream_providers(id,name,type,enabled,weight,daily_limit,quota_timezone,created_at,updated_at) values(1,'默认腾讯云 SES','tencent',1,100,0,'Asia/Shanghai',?,?)`, now(), now()); err != nil {
			return err
		}
...
func (a *App) ensureColumn(table, column, stmt string) error {
